...
MozApp::MozApp(const char* aProfilePath)
{
    ...
    if (NS_FAILED(rv)) {
#if MOZILLA_VERSION_1 <1 //old gecko api
        NS_ABORT("Embedding initialization failed!");
#else
                       ...
                       bool aEntireWord, bool aBackwards)
{
    ...
    PRBool result;
#endif
	#if MOZILLA_VERSION_1 < 8 //FIXME do it correct
    nsCOMPtr<nsIDOMWindowInternal> dom_window_internal =
	#else
	nsCOMPtr<nsIDOMWindow> dom_window_internal =
	#endif
